 ...
 *
 * Changed by Kai Lauterbach 2010-02-10
 *
 */
...
/* This macro defines that the microcontroller only start the bootloader in
 * case of an external reset (reset pin set high). Uncomment this to
 * jump direct into the bootloader
 */
#define USE_EXTERNAL_RESET 1
...
#define JUMPER_BIT  PB0   /* jumper is connected to this bit in port D, active low. If not required uncomment it. */
...
// Required for atmega168
#ifdef __AVR_ATmega168__
    MCUCR = (1<<IVCE);
    MCUCR = (1<<IVSEL);
#endif
// Required for atmega32
...
// only check jumper bit if it was defined.
#ifdef JUMPER_BIT
